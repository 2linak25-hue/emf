...
heading = doc.add_heading('1. Transmission Line Parameters', 1)
...
heading = doc.add_heading('2. Sample Distributions', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
reasons = [
    ...
    'Mean 25 degrees C = global average temperature',
    'Std 10 degrees C = realistic seasonal variation',
    'Range -10 to 45 degrees C covers most climates'
]
for reason in reasons:
    doc.add_paragraph(reason, style='List Bullet')
...
doc.add_paragraph('Transmission lines do not operate at constant current', style='List Bullet')
...
heading = doc.add_heading('3. Physics Equations', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)

doc.add_heading('3.1 Why Simplified Electric Field Formula?', 2)
formula = doc.add_paragraph()
formula.add_run('E = V / (2 * pi * epsilon_0 * r * h)').italic = True
formula.alignment = WD_ALIGN_PARAGRAPH.CENTER

p = doc.add_paragraph()
p.add_run('Reasons:\n').bold = True
reasons = [
    'Derived from Gauss Law for cylindrical conductors',
...
formula = doc.add_paragraph()
formula.add_run('H = I / (2 * pi * d)').italic = True
formula.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
p = doc.add_paragraph()
p.add_run('Temperature Effect (+0.1% per degree C):\n').bold = True
reasons = [
...
heading = doc.add_heading('4. Noise and Outliers', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
reasons = [
    ...
    'Tests ML model outlier detection capability'
]
for reason in reasons:
    doc.add_paragraph(reason, style='List Bullet')
...
heading = doc.add_heading('5. Visualization Choices', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
p.add_run('Reasons:\n').bold = True
reasons = [
    'EMF follows inverse distance law (1/r or 1/r squared)',
...
heading = doc.add_heading('6. Technical Implementation Choices', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
reasons = [
    ...
    '42 = Hitchhiker Guide reference (tradition in ML community)'
]
for reason in reasons:
    doc.add_paragraph(reason, style='List Bullet')
...
heading = doc.add_heading('7. Overall Design Philosophy', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
header_cells[1].text = 'Justification'
...
heading = doc.add_heading('8. Scientific Validation', 1)
for run in heading.runs:
    run.font.color.rgb = RGBColor(0, 102, 204)
...
print(f"Total pages: approximately 15")
